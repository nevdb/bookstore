...
const TestComponent = () => {
  ...
  const { books, loading, error, pagination, fetchBooks, searchBooks, filterBooks } = context;
...
describe('BooksContext', () => {
  ...
  it('handles filter request', async () => {
    ...
    await waitFor(() => {
      expect(mockFilter).toHaveBeenCalledWith({ genre_id: '1' }, 1);
      expect(screen.getByText('Book Count: 2')).toBeInTheDocument();
    });
  });
  ...
  it('provides correct context to child components', () => {
    const ContextConsumer = () => {
      ...
      return <div>{hasRequiredMethods ? 'Valid Context' : 'Invalid Context'}</div>;
    };
  ...
  it('maintains state across multiple operations', async () => {
    ...
    mockGetBooks.mockResolvedValueOnce({
      data: {
        data: [
          { id: 3, title: 'New Book', genre: {}, author: {} },
        ],
        pagination: { current_page: 2, total: 3, last_page: 2 },
      },
    });
