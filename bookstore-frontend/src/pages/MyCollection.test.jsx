...
vi.mock('../components/CollectionItem', () => ({
  default: ({ userBook, onUpdate, onRemove }) => (
    <div data-testid={`collection-item-${userBook.id}`}>
      ...
      <button data-testid={`edit-${userBook.id}`} onClick={() => onUpdate(userBook.id, {})}>
        Edit
      </button>
      <button data-testid={`remove-${userBook.id}`} onClick={() => onRemove(userBook.id)}>
        Remove
      </button>
    </div>
  )
...
describe('MyCollection', () => {
  const mockUserBooks = [
    {
      id: 1,
      book: { id: 1, title: 'Book 1', author: { name: 'Author 1' }, genre: { name: 'Fiction' } },
      personal_rating: 5,
      status: 'completed',
      notes: 'Great book'
    },
    {
      id: 2,
      book: { id: 2, title: 'Book 2', author: { name: 'Author 2' }, genre: { name: 'Mystery' } },
      personal_rating: 4,
      status: 'reading',
      notes: 'Still reading'
    }
  ...
  it('displays empty message when collection is empty', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Your collection is empty. Start adding books!')).toBeInTheDocument();
    });
  });
  ...
  it('switches pages when pagination button clicked', async () => {
    ...
    await waitFor(() => {
      expect(collectionService.default.getCollection).toHaveBeenLastCalledWith(2);
    });
  });

  it('displays error message when fetch fails', async () => {
    collectionService.default.getCollection.mockRejectedValue(new Error('API Error'));
